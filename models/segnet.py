...
from tools.helpers import set_trainable


class SegNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, freeze_backbone=False, path_model=None):
        ...
        # features = 前面的卷积模块（Conv / BN / Relu / MaxPool）
        # .children()的作用：返回当前模块内部的直接子模块， Python生成器
...
class SegResNet(nn.Module):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, path_model=None):
        super(SegResNet, self).__init__()
        resnet50 = models.resnet50()
        if path_model:
            resnet50.load_state_dict(torch.load(path_model, map_location="cpu"))
            print("load pretrain model done!")
...
if __name__ == "__main__":
    ...
    model = SegNet(num_classes=21, path_model=0)
