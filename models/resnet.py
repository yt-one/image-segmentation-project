...
def resnet18(path_model=None, **kwargs):
...
def resnet34(path_model=None, **kwargs):
...
def resnet50(path_model=None, **kwargs):
    ...
    if path_model is not None:
        if os.path.exists(path_model):
            ...
            model.load_state_dict(load_url(model_urls['resnet50'], model_dir=model_dir))
        else:
            raise Exception("model_path is not exists!!\n{}".format(os.path.abspath(path_model)))
    return model


def resnet101(path_model=None, **kwargs):
    ...
    if path_model is not None:
        if os.path.exists(path_model):
            model_dir = os.path.dirname(path_model)
            model.load_state_dict(load_url(model_urls['resnet101'], model_dir=model_dir))
        else:
            raise Exception("model_path is not exists!!\n{}".format(os.path.abspath(path_model)))
    return model


def resnet152(path_model=None, **kwargs):
    ...
    if path_model:
        model_dir = os.path.dirname(path_model)
        model.load_state_dict(load_url(model_urls['resnet152'], model_dir=model_dir))
    return model
...
def load_url(url, model_dir='./pretrained', map_location="cpu"):
    ...
    cached_file_pth = os.path.join(model_dir, filename + '.pth')
    cached_file_zip = os.path.join(model_dir, filename + '.zip')

    if os.path.exists(cached_file_pth):
        cached_file = os.path.join(model_dir, filename + '.pth')
        return torch.load(cached_file, map_location=map_location)

    if not os.path.exists(cached_file_zip):
        sys.stderr.write('Downloading: "{}" to {}\n'.format(url, cached_file_zip))
        urlretrieve(url, cached_file_zip)

    zip_ref = zipfile.ZipFile(cached_file_zip, 'r')
