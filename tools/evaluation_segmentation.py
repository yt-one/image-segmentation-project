...
def calc_semantic_segmentation_confusion(pred_labels, gt_labels, n_class):
    ...
    for pred_label, gt_label in six.moves.zip(pred_labels, gt_labels):
        if pred_label.ndim != 2 or gt_label.ndim != 2:
            raise ValueError('ndim of labels should be two. pred.ndim:{}, gt.ndim:{}'.format(
                pred_label.ndim, gt_label.ndim))
        ...
        # 把2d矩阵变为1d进行编号排序，n_cls*gt 是确定在哪一行，+pred 表示真实标签在这一行，然后预测为第几个
        # 统计完成后再reshape回2d矩阵
        mask = gt_label >= 0
...
def calc_semantic_segmentation_iou(confusion):
    ...
    * :math:`\\text{IoU of the i-th class} =  \
    ...
    iou_denominator = (confusion.sum(axis=1) + confusion.sum(axis=0)
                       - np.diag(confusion))
    iou = np.diag(confusion) / iou_denominator  # 预测正确的数量 / 真实的数量+预测为该类的数量，即  交集/并集
    return iou
...
def eval_semantic_segmentation(pred_labels, gt_labels, n_class):
    ...
    * :math:`\\text{Mean Class Accuracy} = \\frac{1}{k} \
    ...
    confusion = calc_semantic_segmentation_confusion(pred_labels, gt_labels, n_class)
